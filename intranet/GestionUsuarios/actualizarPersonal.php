...
if (isset($_POST['save_Personal']) && $_POST['accion'] == "Agregar") {
  ...
  $stmt3->close();
  //CREAR ROL SIsTEMA
  $result_code = $conn->query("SELECT COD_USUARIO FROM USUARIO WHERE COD_PERSONA=" . $COD_PERSONA);
  $row2 = mysqli_fetch_assoc($result_code);
  $COD_USUARIO = $row2['COD_USUARIO'];
  $stmt4 = $conn->prepare("INSERT INTO ROL_USUARIO (COD_ROL, COD_USUARIO,ESTADO) VALUES (?,?,?)");
  $stmt4->bind_param('iis', $COD_ROL, $COD_USUARIO, $ESTADO);
