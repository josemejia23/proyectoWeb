...
class DocenteService extends MainService{
    ...
    function updateNotasAlumnoPrimer($nota1,$nota2,$nota3, $nota4, $nota5,$codAsigPeriodo) {
        $stmt = $this->conex->prepare("UPDATE ALUMNO_ASIGNATURA_PERIODO set NOTA1=?,NOTA2=?, NOTA3=?, NOTA4=?,NOTA5=?
        WHERE COD_ASIG_PERIODO= ?");
        $stmt->bind_param("dddddi", $nota1,$nota2,$nota3, $nota4, $nota5,$codAsigPeriodo);
        $stmt->execute();
        $stmt->close();
    }
    function updateNotasAlumnoSegundo($nota6, $nota7, $nota8, $nota9, $nota10, $nota11, $codAsigPeriodo) {
        $stmt = $this->conex->prepare("UPDATE ALUMNO_ASIGNATURA_PERIODO set NOTA6=?, NOTA7=?, NOTA8=?, NOTA9=?, NOTA10=?, NOTA11=?
        WHERE COD_ASIG_PERIODO= ?");
        $stmt->bind_param("ddddddi", $nota6, $nota7, $nota8, $nota9, $nota10, $nota11, $codAsigPeriodo);
        $stmt->execute();
        $stmt->close();
    }
